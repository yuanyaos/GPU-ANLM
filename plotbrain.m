...
for i=1:size(nanidx,2)
    if(i==1)
        plot(cutpos(cutloop(1:(nanidx(i)-1)),1),223-cutpos(cutloop(1:(nanidx(i)-1)),3),'color','k','LineWidth',1);
        hold on
    else
        plot(cutpos(cutloop((nanidx(i-1)+1):(nanidx(i)-1)),1),223-cutpos(cutloop((nanidx(i-1)+1):(nanidx(i)-1)),3),'color','k','LineWidth',1);
        hold on
    end
end
